...
# Ensure every product has default shopee_affiliate_url
for p in data["products"]:
    if not p.get("shopee_affiliate_url"):
        p["shopee_affiliate_url"] = "https://th.shp.ee/sdFv2cS1"
...
# Build pure cream & shopee orange HTML
...
<body class="bg-[#F9F6F0] text-[#2C241E] min-h-screen flex flex-col font-sans">
    ...
    <!-- HEADER (CREAM WHITE & SHOPEE ORANGE) -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-3 cursor-pointer" onclick="showCatalogView()">
                    <div class="w-10 h-10 rounded-2xl bg-[#EE4D2D] flex items-center justify-center text-white font-black text-xl shadow-md shadow-orange-500/20">
                        🎯
                    </div>
                    <div>
                        <span class="font-black text-lg tracking-wide text-[#2C241E]">GOODSTONE</span>
                        <span class="text-[11px] block text-slate-500 font-medium">ร้านหนังสติ๊กยุทธวิธีและอุปกรณ์</span>
                    </div>
                </div>

                <!-- Nav Menu -->
                <nav class="flex items-center gap-4 sm:gap-6 text-xs sm:text-sm font-bold">
                    <button onclick="showCatalogView()" class="text-[#2C241E] hover:text-[#EE4D2D] transition-colors flex items-center gap-1.5">
                        <i data-lucide="store" class="w-4 h-4 text-[#EE4D2D]"></i>
                        <span>หน้าร้านค้า</span>
                    </button>
                    <a href="track.html" class="text-slate-600 hover:text-[#EE4D2D] transition-colors flex items-center gap-1.5">
    ...
    <div class="bg-[#EE4D2D] text-white py-2 px-4 text-center text-xs sm:text-sm font-bold shadow-sm">
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-8">
        ...
        <section id="view-catalog" class="space-y-6">
            ...
            <!-- Hero Banner (Warm Cream & Shopee Orange) -->
            <div class="rounded-3xl bg-gradient-to-r from-[#FFF6F2] via-[#FDF3EA] to-[#FBF0E4] p-6 sm:p-10 border-2 border-[#EBE3D5] shadow-sm space-y-3">
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FFF0EB] text-[#EE4D2D] border border-[#FFD5CC] text-xs font-extrabold">
                    <span>🔥 หนังสติ๊กยุทธวิธีเกรดพรีเมียม & อุปกรณ์ครบวงจร</span>
                </div>
                <h1 class="text-2xl sm:text-4xl font-black text-[#2C241E] leading-tight">
                    หนังสติ๊กยุทธวิธี ยางแบนแรงสูง <br><span class="text-[#EE4D2D]">จัดส่งด่วน EMS / SPX ทั่วประเทศ</span>
                </h1>
            ...
            <!-- Categories Filter & Search (Cream & White Theme) -->
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-3xl border border-[#EBE3D5] shadow-sm">
                <div class="flex flex-wrap gap-2 w-full sm:w-auto" id="category-tabs">
                    <button onclick="filterCategory('all')" class="cat-btn active px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#EE4D2D] text-white shadow-md" data-cat="all">ทั้งหมด</button>
                    <button onclick="filterCategory('slingshot')" class="cat-btn px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#F9F6F0] text-slate-700 hover:bg-[#FFF2EE] hover:text-[#EE4D2D] border border-[#EBE3D5]" data-cat="slingshot">🎯 หนังสติ๊ก</button>
                    <button onclick="filterCategory('rubber')" class="cat-btn px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#F9F6F0] text-slate-700 hover:bg-[#FFF2EE] hover:text-[#EE4D2D] border border-[#EBE3D5]" data-cat="rubber">⚡ ยางหนังสติ๊ก</button>
                    <button onclick="filterCategory('ammo')" class="cat-btn px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#F9F6F0] text-slate-700 hover:bg-[#FFF2EE] hover:text-[#EE4D2D] border border-[#EBE3D5]" data-cat="ammo">🔘 ลูกเหล็ก/กระสุน</button>
                    <button onclick="filterCategory('accessories')" class="cat-btn px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#F9F6F0] text-slate-700 hover:bg-[#FFF2EE] hover:text-[#EE4D2D] border border-[#EBE3D5]" data-cat="accessories">🎒 อุปกรณ์เสริม/เลเซอร์</button>
                </div>
                <div class="relative w-full sm:w-64">
                    <input type="text" id="search-input" onkeyup="searchProducts()" placeholder="ค้นหาสินค้า..." class="w-full pl-10 pr-4 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-2xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-[#EE4D2D] focus:bg-white">
                    <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3.5 top-3"></i>
                </div>
            </div>

            <!-- Product Grid (White Cards with Cream Borders) -->
        ...
        <section id="view-checkout" class="hidden space-y-6">
            ...
            <div class="flex items-center justify-between">
                <button onclick="showCatalogView()" class="inline-flex items-center gap-2 bg-white hover:bg-[#FFF2EE] text-[#2C241E] hover:text-[#EE4D2D] px-4 py-2 rounded-2xl border-2 border-[#EBE3D5] font-bold text-xs sm:text-sm transition-all shadow-sm active:scale-95">
                    <span>← กลับไปดูสินค้าทั้งหมด</span>
                </button>
                <span class="text-xs font-black text-[#EE4D2D] bg-[#FFF2EE] px-3.5 py-1 rounded-full border border-[#FFD5CC]">
                    ⚡ สั่งซื้อด่วน (Direct Checkout)
                </span>
            </div>

            <!-- Direct Checkout Panel (Cream-White Cards) -->
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">

                <!-- LEFT: MULTI-IMAGE GALLERY & PRODUCT DETAILS (5 Cols) -->
                <div class="lg:col-span-5 bg-white p-5 sm:p-6 rounded-3xl border-2 border-[#EBE3D5] shadow-sm space-y-5">
                    
                    <!-- Main Image Area -->
                    <div onclick="openGalleryModal()" class="relative h-64 sm:h-80 bg-[#FAF7F2] rounded-2xl overflow-hidden cursor-pointer group flex items-center justify-center border border-[#EBE3D5]">
                        ...
                        <span id="detail-stock-badge" class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-[#EE4D2D] border border-orange-200 text-xs px-3 py-1 rounded-full font-black shadow-sm">
                            สต็อก: 35 ชิ้น
                        </span>
                        <div class="absolute bottom-3 left-3 bg-white/95 text-[#2C241E] border border-slate-200 text-[11px] px-3 py-1.5 rounded-xl flex items-center gap-1.5 font-bold group-hover:bg-[#EE4D2D] group-hover:text-white group-hover:border-[#EE4D2D] transition-all shadow-sm">
                    ...
                    <div class="space-y-2 pt-2 border-t border-slate-100">
                        <span id="detail-cat-tag" class="px-2.5 py-0.5 rounded-full bg-[#FFF2EE] text-[#EE4D2D] border border-[#FFD5CC] text-xs font-extrabold uppercase">
                            SLINGSHOT
                        </span>
                        <h1 id="detail-title" class="text-xl sm:text-2xl font-black text-[#2C241E] leading-snug">
                            หนังสติ๊กอัลลอยด์ยุทธวิธี
                        </h1>
                        <p id="detail-desc" class="text-xs sm:text-sm text-slate-600 leading-relaxed">
                    ...
                    <div class="space-y-2.5 pt-3 border-t border-slate-100">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-[10px] text-slate-500 bg-[#F9F6F0] px-2 py-0.5 rounded font-semibold">เลือกได้ทันที</span>
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-slate-100">
                        ...
                        <div class="flex items-center gap-2 bg-[#F9F6F0] px-3 py-1.5 rounded-xl border border-[#EBE3D5]">
                            <button onclick="changeQty(-1)" class="text-slate-700 hover:text-red-600 font-bold px-2 text-sm">-</button>
                            <span id="detail-qty-display" class="font-bold text-sm text-[#2C241E] w-6 text-center">1</span>
                ...
                <div class="lg:col-span-7 bg-white p-6 sm:p-8 rounded-3xl border-2 border-[#EBE3D5] shadow-sm space-y-6">
                    
                    <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                        <div>
                            <h2 class="text-lg sm:text-xl font-black text-[#2C241E] flex items-center gap-2">
                                <span>⚡</span> กรอกที่อยู่ & ชำระเงิน (Guest Checkout)
                            </h2>
                            <p class="text-xs text-slate-500">ไม่ต้องสมัครสมาชิก กรอกที่อยู่แล้วชำระเงินได้ทันที</p>
                        </div>
                        <div id="auto-cookie-badge" class="hidden text-right">
                            <span class="text-[10px] text-emerald-800 bg-emerald-50 border border-emerald-300 px-2.5 py-1 rounded-lg font-bold">
                    ...
                    <!-- Step 1: Address Form -->
                    <div class="space-y-3">
                        <h3 class="text-xs font-bold text-[#2C241E] uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="col-span-2 sm:col-span-1">
                                ...
                                <input type="text" id="cust-name" placeholder="ระบุชื่อและนามสกุล..." class="w-full px-3.5 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-xl text-xs sm:text-sm focus:ring-2 focus:ring-[#EE4D2D] focus:bg-white">
                            </div>

                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-xs font-bold text-slate-700 mb-1">เบอร์โทรศัพท์มือถือ *</label>
                                <input type="tel" id="cust-phone" oninput="onPhoneChange(this.value)" placeholder="ระบุเบอร์โทรติดต่อ..." class="w-full px-3.5 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-xl text-xs sm:text-sm focus:ring-2 focus:ring-[#EE4D2D] focus:bg-white">
                            </div>

                            <div class="col-span-2">
                                <label class="block text-xs font-bold text-slate-700 mb-1">บ้านเลขที่ / ซอย / ถนน *</label>
                                <input type="text" id="cust-address-line" placeholder="เช่น 123/45 ซอย 8 ถนนพระราม 2..." class="w-full px-3.5 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-xl text-xs sm:text-sm focus:ring-2 focus:ring-[#EE4D2D] focus:bg-white">
                            </div>
                            ...
                            <div class="col-span-2 sm:col-span-1 relative">
                                ...
                                <input type="text" id="cust-postcode" maxlength="5" oninput="handlePostalCodeInput(this.value)" placeholder="เช่น 10150, 84320..." class="w-full px-3.5 py-2.5 bg-[#FFF6F2] border-2 border-[#FFD5CC] rounded-xl text-xs sm:text-sm font-bold text-slate-900 focus:ring-2 focus:ring-[#EE4D2D] focus:bg-white">
                                <div id="address-suggestions-box" class="hidden absolute left-0 right-0 top-full mt-1 bg-white border border-[#EBE3D5] rounded-xl shadow-xl z-20 max-h-48 overflow-y-auto"></div>
                            </div>

                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-xs font-bold text-slate-700 mb-1">ตำบล / แขวง</label>
                                <input type="text" id="cust-subdistrict" placeholder="ตำบล/แขวง" class="w-full px-3.5 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-xl text-xs sm:text-sm">
                            </div>

                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-xs font-bold text-slate-700 mb-1">อำเภอ / เขต</label>
                                <input type="text" id="cust-district" placeholder="อำเภอ/เขต" class="w-full px-3.5 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-xl text-xs sm:text-sm">
                            </div>

                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-xs font-bold text-slate-700 mb-1">จังหวัด</label>
                                <input type="text" id="cust-province" placeholder="จังหวัด" class="w-full px-3.5 py-2.5 bg-[#F9F6F0] border border-[#EBE3D5] rounded-xl text-xs sm:text-sm">
                            </div>
                        </div>

                        <!-- Logistics Routing Box -->
                        <div id="logistics-routing-box" class="bg-[#FAF7F2] border border-[#EBE3D5] rounded-2xl p-3.5 text-xs space-y-1">
                            <div class="flex items-center justify-between font-bold text-slate-900">
                                ...
                                <span id="carrier-fee-badge" class="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 font-bold">
                                    ส่งฟรี (฿0)
                                </span>
                            </div>
                    ...
                    <div class="space-y-4 pt-3 border-t border-slate-100">
                        ...
                        <!-- PROMPTPAY PANEL -->
                        <div id="panel-promptpay" class="bg-[#FAF7F2] border border-[#EBE3D5] rounded-2xl p-4 sm:p-5 space-y-4 text-center">
                            <div class="space-y-1">
                            ...
                            <div class="bg-white p-3 rounded-2xl border-2 border-[#EBE3D5] shadow-md inline-block">
                                <img id="promptpay-qr-img" src="" alt="PromptPay QR" class="w-44 h-44 object-contain">
                            </div>

                            <div class="bg-white p-3 rounded-xl border border-[#EBE3D5] text-xs text-left space-y-1">
                            ...
                            <div class="space-y-2 text-left pt-2 border-t border-[#EBE3D5]">
                    ...
                    <!-- Pricing Summary -->
                    <div class="p-4 bg-[#FAF7F2] rounded-2xl border border-[#EBE3D5] space-y-2 text-xs">
                        <div class="flex justify-between text-slate-600">
                            <span>ราคาสินค้า (<span id="summary-variant-name">รุ่นมาตรฐาน</span> x<span id="summary-qty">1</span>)</span>
                            <span id="summary-subtotal" class="font-bold text-[#2C241E]">฿390.00</span>
                        </div>
                        <div class="flex justify-between text-slate-600">
                            <span>ค่าจัดส่ง (<span id="summary-carrier">SPX Express</span>)</span>
                            <span id="summary-shipping" class="font-bold text-[#2C241E]">ฟรี (฿0)</span>
                        </div>
                        <div class="flex justify-between text-base font-black text-[#2C241E] pt-2 border-t border-[#EBE3D5]">
                    ...
                    <!-- ACTION 2: SOCIAL PROOF SHOPEE AFFILIATE (In Checkout View) -->
                    <div id="shopee-affiliate-box" class="text-center pt-1">
                        <a id="shopee-affiliate-btn" href="https://th.shp.ee/sdFv2cS1" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-slate-700 hover:text-[#EE4D2D] bg-[#FAF7F2] hover:bg-[#FFF2EE] px-4 py-2.5 rounded-2xl border border-[#EBE3D5] transition-all">
    ...
    <div id="topup-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm hidden">
        <div class="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            <div class="flex justify-between items-center border-b border-slate-100 pb-3">
                <h3 class="font-black text-[#2C241E] text-base">👛 เติมเงินกระเป๋าเครดิต (รับโบนัสพิเศษ)</h3>
            ...
            <div class="space-y-2.5">
                <div onclick="selectTopup(300, 10)" class="p-3.5 rounded-2xl border-2 border-[#EBE3D5] hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer flex justify-between items-center transition-all">
                    <div>
                        <span class="text-sm font-bold text-[#2C241E] block">เติม ฿300</span>
                        <span class="text-xs text-emerald-700 font-bold">รับโบนัส +฿10 (ได้เครดิต ฿310)</span>
                    </div>
                    <span class="bg-emerald-600 text-white text-xs px-3 py-1.5 rounded-xl font-bold">เลือก</span>
                </div>
                <div onclick="selectTopup(500, 30)" class="p-3.5 rounded-2xl border-2 border-[#EBE3D5] hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer flex justify-between items-center transition-all">
                    <div>
                        <span class="text-sm font-bold text-[#2C241E] block">เติม ฿500 (ยอดนิยม)</span>
                        <span class="text-xs text-emerald-700 font-bold">รับโบนัส +฿30 (ได้เครดิต ฿530)</span>
                    </div>
                    <span class="bg-emerald-600 text-white text-xs px-3 py-1.5 rounded-xl font-bold">เลือก</span>
                </div>
                <div onclick="selectTopup(1000, 80)" class="p-3.5 rounded-2xl border-2 border-[#EBE3D5] hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer flex justify-between items-center transition-all">
                    <div>
                        <span class="text-sm font-bold text-[#2C241E] block">เติม ฿1,000</span>
                        <span class="text-xs text-emerald-700 font-bold">รับโบนัส +฿80 (ได้เครดิต ฿1,080)</span>
                    </div>
                    <span class="bg-emerald-600 text-white text-xs px-3 py-1.5 rounded-xl font-bold">เลือก</span>
                </div>
            </div>
    ...
    <!-- FOOTER -->
    <footer class="bg-white border-t-2 border-[#EBE3D5] text-slate-500 py-6 text-xs text-center space-y-1.5">
        <p class="font-bold text-[#2C241E]">GOODSTONE TACTICAL SLINGSHOT © 2026</p>
    ...
    <script>
        ...
        function renderCatalogGrid() {
            ...
            list.forEach(p => {
                ...
                card.className = "bg-white rounded-3xl shadow-sm hover:shadow-md border-2 border-[#EBE3D5] overflow-hidden flex flex-col transition-all group";

                card.innerHTML = `
                    <!-- Clickable Product Image triggers Direct Checkout -->
                    <div onclick="openProductDirectCheckout('${p.id}')" class="h-48 sm:h-52 overflow-hidden bg-[#FAF7F2] relative flex items-center justify-center cursor-pointer group/img border-b border-[#EBE3D5]">
                        ...
                        <span class="absolute top-3 right-3 bg-white/95 text-[#EE4D2D] border border-orange-200 text-[11px] px-2.5 py-1 rounded-full font-black shadow-sm">
                            คงเหลือ ${p.stock}
                        </span>
                        <div class="absolute bottom-2 left-2 bg-white/95 text-[#2C241E] border border-slate-200 text-[10px] px-2.5 py-1 rounded-xl flex items-center gap-1 group-hover/img:bg-[#EE4D2D] group-hover/img:text-white group-hover/img:border-[#EE4D2D] font-bold transition-all shadow-sm">
                    ...
                    <div class="p-4 sm:p-5 flex flex-col flex-grow justify-between space-y-3">
                        <div class="space-y-1.5">
                            <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-[#FFF2EE] text-[#EE4D2D] border border-[#FFD5CC]">
                                ${p.category}
                            </span>
                            <h3 onclick="openProductDirectCheckout('${p.id}')" class="font-bold text-[#2C241E] text-sm sm:text-base line-clamp-2 cursor-pointer hover:text-[#EE4D2D] transition-colors">
                                ${p.name}
                            </h3>
                        ...
                        <div class="pt-2 border-t border-slate-100 space-y-2">
                            ...
                            <!-- Dedicated Shopee Review Button directly on Card -->
                            <a href="${shopeeUrl}" target="_blank" rel="noopener noreferrer" class="w-full text-center block bg-[#FFF2EE] hover:bg-[#FFE3DC] text-[#EE4D2D] py-1.5 rounded-xl border border-[#FFD5CC] text-xs font-bold transition-colors">
                                ⭐ ดูรีวิวใน Shopee &gt;
                            </a>
                        </div>
                    </div>
        ...
        function filterCategory(cat) {
            ...
            document.querySelectorAll(".cat-btn").forEach(btn => {
                if (btn.dataset.cat === cat) {
                    btn.className = "cat-btn active px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#EE4D2D] text-white shadow-md";
                } else {
                    btn.className = "cat-btn px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition-all bg-[#F9F6F0] text-slate-700 hover:bg-[#FFF2EE] hover:text-[#EE4D2D] border border-[#EBE3D5]";
                }
            });
            renderCatalogGrid();
        }
        ...
        function searchProducts() {
            ...
            filtered.forEach(p => {
                const shopeeUrl = p.shopee_affiliate_url || "https://th.shp.ee/sdFv2cS1";
                const card = document.createElement("div");
                card.className = "bg-white rounded-3xl shadow-sm hover:shadow-md border-2 border-[#EBE3D5] overflow-hidden flex flex-col transition-all";
                card.innerHTML = `
                    <div onclick="openProductDirectCheckout('${p.id}')" class="h-48 overflow-hidden bg-[#FAF7F2] relative flex items-center justify-center cursor-pointer border-b border-[#EBE3D5]">
                    ...
                    <div class="p-4 flex flex-col flex-grow justify-between space-y-2">
                        <h3 onclick="openProductDirectCheckout('${p.id}')" class="font-bold text-[#2C241E] text-sm cursor-pointer hover:text-[#EE4D2D]">${p.name}</h3>
                        ...
                        <a href="${shopeeUrl}" target="_blank" class="w-full text-center block bg-[#FFF2EE] text-[#EE4D2D] py-1.5 rounded-xl border border-[#FFD5CC] text-xs font-bold">
                            ⭐ ดูรีวิวใน Shopee &gt;
                        </a>
                    </div>
                `;
        ...
        function renderProductCheckoutDetail() {
            ...
            images.forEach((img, idx) => {
                ...
                thumb.className = `w-14 h-14 rounded-xl p-1 bg-[#FAF7F2] border-2 cursor-pointer transition-all flex-shrink-0 flex items-center justify-center ${idx === 0 ? "border-[#EE4D2D] scale-105 shadow-md" : "border-[#EBE3D5] opacity-70"}`;
                ...
                thumb.onclick = () => {
                    ...
                    Array.from(thumbsStrip.children).forEach((c, i) => {
                        c.className = `w-14 h-14 rounded-xl p-1 bg-[#FAF7F2] border-2 cursor-pointer transition-all flex-shrink-0 flex items-center justify-center ${i === idx ? "border-[#EE4D2D] scale-105 shadow-md" : "border-[#EBE3D5] opacity-70"}`;
                    });
                };
            ...
            p.variants.forEach((v, idx) => {
                ...
                btn.type = "button";
                btn.className = `px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${isSel ? "bg-[#EE4D2D] text-white border-2 border-[#d73211] shadow-md scale-105" : "bg-[#F9F6F0] hover:bg-[#FFF2EE] hover:border-[#EE4D2D] text-[#2C241E] border border-[#EBE3D5]"}`;
                btn.innerHTML = `${isSel ? "<span>✓</span>" : ""}<span>${v.name}</span><span class="${isSel ? "bg-black/30 text-amber-300" : "bg-[#FFF2EE] text-[#EE4D2D]"} text-[11px] px-1.5 py-0.5 rounded font-black">฿${v.price.toLocaleString()}</span>`;
                btn.onclick = () => {
...
print("slingshot-shop/index.html updated with true Cream & Shopee Orange theme & Shopee review button on every card!")
